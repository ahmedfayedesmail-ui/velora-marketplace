...
--
-- Recorded in Restore-Test migration history as:
-- 20260926180652 / checkout_localization_catalog_20260926
...
select key,source_text,'ui',true,now() from translations
...
with translations(key, translated_text) as (
  values
    ('Shipping Information','معلومات الشحن'),('Full Name','الاسم الكامل'),('Phone','الهاتف'),
    ('Email','البريد الإلكتروني'),('Country / Region','الدولة / المنطقة'),('Select country','اختر الدولة'),
    ('Loading countries…','جارٍ تحميل الدول…'),('Egypt','مصر'),('City','المدينة'),
    ('Address','العنوان'),('Notes (optional)','ملاحظات (اختياري)'),('Payment Method','طريقة الدفع'),
    ('Card','بطاقة'),('Card payment','الدفع بالبطاقة'),('Cash on Delivery','الدفع عند الاستلام'),
    ('Pay when you receive','الدفع عند الاستلام'),('Summary','ملخص الطلب'),('Qty','الكمية'),
    ('Place Order','إتمام الطلب'),('Loading checkout','جارٍ تحميل الدفع'),
    ('Syncing your cart…','جارٍ مزامنة سلة التسوق…'),('Preparing your order summary…','جارٍ تجهيز ملخص طلبك…'),
    ('Free','مجانًا'),('Please complete your shipping information','يرجى استكمال بيانات الشحن'),
    ...
    ('Shipping quote unavailable.','تعذر احتساب تكلفة الشحن حاليًا.'),('Shipping quote ready.','تم احتساب تكلفة الشحن.'),
    ('Shipping configuration required for one or more sellers.','تحتاج إعدادات الشحن لبعض المتاجر.'),
    ('Gift card','بطاقة هدايا'),('Gift card code','رمز بطاقة الهدايا'),
    ('Gift card applied:','تم تطبيق بطاقة الهدايا:'),('Remaining:','المتبقي:'),('Gift card removed.','تمت إزالة بطاقة الهدايا')
)
insert into public.velora_i18n_translations(key,locale,translated_text,is_machine_generated,is_reviewed,updated_at)
select key,'ar',translated_text,false,true,now() from translations
