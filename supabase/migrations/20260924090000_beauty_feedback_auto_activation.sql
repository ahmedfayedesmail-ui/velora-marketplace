-- Velora S1-D — automatically activate validated beauty feedback.
-- Scope: Restore-Test / staging only. Production remains frozen.
--
-- Existing server-side validation remains authoritative:
-- auth, product/rating/texture/effect, source, purchase eligibility,
-- and idempotency checks must all pass before the row is inserted.
-- Successfully validated feedback is now approved immediately so routine
-- evolution does not depend on a manual staff action.
...
begin
  ...
  values (
    ...
    trim(p_idempotency_key), 'approved'
  )
