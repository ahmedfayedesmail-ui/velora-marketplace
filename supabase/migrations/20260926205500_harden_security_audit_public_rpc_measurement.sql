-- VELORA — make security attack-surface RPC exposure measurement evidence-based
-- Public read RPCs and explicitly guarded invoker RPCs are not equivalent to
-- an anonymously reachable privileged writer. Keep exposure visible, but only
-- fail when public/anon execution exposes unguarded mutation text.
    ...
    select * from (
      values
        ...
        (
          ...
          format(
            'Security-definer functions without explicit public search_path pin: %s',
            v_bad_search_path
          )
        )
    ) q(k,s,m)
