...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(x) order by x.created_at desc), '[]'::jsonb)
    into v_routines
  from (
    select id, contract_version, ruleset_version, catalog_revision,
           input_fingerprint, status, created_at
    from public.beauty_routine_runs
    where user_id = v_uid
    order by created_at desc
    limit 5
  ...
  if v_run.id is not null then
    select coalesce(jsonb_agg(to_jsonb(s) order by s.step_order), '[]'::jsonb)
      into v_steps
    from public.beauty_routine_steps s
    where s.routine_run_id = v_run.id;
  end if;
