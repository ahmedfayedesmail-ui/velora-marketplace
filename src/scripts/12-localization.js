...
(function(){
  ...
  async function openCanonicalAdmin(){
    ...
    // The Admin shell must never wait for locale/auth network work. Mount the
    // protected UI immediately, then validate the current session and roles.
    // This removes the async language/runtime path from the visual entry gate.
    ...
    try{
      window.VELORA_ADMIN_ROLES=Array.isArray(window.VELORA_ADMIN_ROLES)
        ? window.VELORA_ADMIN_ROLES : [];
      p.innerHTML=canonicalAdminLayout();
      p.classList.add('active');
      document.body.style.overflow='hidden';
      try{window.VELORA_I18N_RENDER?.(p);}catch(_){}
      scheduleAdminPostMountTrace();
      platformTrace('openCanonicalAdmin shell mounted',{
        active:true,
      ...
      // Re-render once with the verified role label, then load the dashboard.
      p.innerHTML=canonicalAdminLayout();
      p.classList.add('active');
      try{window.VELORA_I18N_RENDER?.(p);}catch(_){}
      ...
      // Preserve a visible error shell so a failed network/RPC cannot look
      // like a dead Admin button.
      const errHost=document.getElementById('adminContent');
      if(errHost){
        errHost.innerHTML='<div class="velora-op-card"><b>Admin could not finish loading.</b><div class="velora-op-muted">'+esc(e?.message||e)+'</div></div>';
        try{window.VELORA_I18N_RENDER?.(errHost);}catch(_){}
      }else{
        toastErr(e);
      }
    }
