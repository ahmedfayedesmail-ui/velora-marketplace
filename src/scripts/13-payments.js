...
async function syncCheckoutCurrencyGate(){const select=document.getElementById('currencySelect');if(!select)return;const country=String(document.getElementById('veloraCountryCode')?.value||window.VELORA_MARKET_CONTEXT?.countryCode||'EG').toUpperCase();try{const {data,error}=await db.rpc('velora_get_operational_checkout_currencies',{p_country_code:country});if(error)throw error;const allowed=(data||[]).map(x=>String(x.currency_code||'').toUpperCase()).filter(Boolean);select.querySelectorAll('option').forEach(o=>{o.disabled=allowed.length? !allowed.includes(String(o.value||'').toUpperCase()):true;});if(allowed.length&&!allowed.includes(String(select.value||'').toUpperCase()))select.value=allowed[0];select.dataset.veloraCheckoutCurrencyLocked='1';select.title=allowed.length?tr('Checkout currently supports:')+' '+allowed.join(', '):tr('No operational checkout currency is currently available.');}catch(e){select.querySelectorAll('option').forEach(o=>{o.disabled=true;});select.title=tr('Checkout currency availability could not be verified.');console.warn('Stage9 checkout currency gate',e)}}
function clearCheckoutCurrencyGate(){const select=document.getElementById('currencySelect');if(!select||select.dataset.veloraCheckoutCurrencyLocked!=='1')return;select.querySelectorAll('option').forEach(o=>{o.disabled=false;});select.dataset.veloraCheckoutCurrencyLocked='0';select.title=tr('Currency');}
...
function decorateGiftCard(){
  ...
  box.style.marginTop='1rem';box.style.display='block';box.style.width='100%';
  box.innerHTML='<div style="font-weight:700;margin-bottom:.4rem;white-space:nowrap">🎁 '+esc(tr('Gift card'))+'</div><div style="display:flex;gap:.5rem;flex-wrap:nowrap;align-items:center"><input id="veloraGiftCardInput" class="coupon-input" style="min-width:0" placeholder="'+esc(tr('Gift card code'))+'" type="text"><button id="veloraGiftCardApply" class="coupon-btn" type="button">'+esc(tr('Apply'))+'</button><button id="veloraGiftCardRemove" class="coupon-btn" type="button" style="display:none">'+esc(tr('Remove'))+'</button></div><div id="veloraGiftCardStatus" class="velora-op-muted" style="margin-top:.4rem;width:100%;line-height:1.5"></div>';
...
/* Canonical placeOrder wrapper: create order first, bind tender, then start provider only when needed. */
async function clearCanonicalCheckoutCart(items){
  const failures=[];
  const rows=Array.isArray(items)?items:[];
  await Promise.all(rows.map(async function(item){
    try{
      const result=item.product_variant_id
        ? await db.rpc('velora_remove_cart_item_variant',{
            p_product_id:item.product_id,
            p_product_variant_id:item.product_variant_id
          })
        : await db.rpc('velora_remove_cart_item',{
            p_product_id:item.product_id
          });
      if(result.error)throw result.error;
    }catch(error){
  ...
  if(failures.length)console.warn('Velora checkout cart cleanup failures',failures);
  return {failed:failures.length,failures};
}

window.placeOrder=async function(event){
  if(window.__VELORA_CHECKOUT_SUBMITTING===true)return;
  window.__VELORA_CHECKOUT_SUBMITTING=true;
  ...
  const fullGiftCard=Boolean(
    ...
    window.VELORA_GIFT_CARD_CODE
  );

  if(event&&typeof event.preventDefault==='function')event.preventDefault();

  if(!selected?.id&&!fullGiftCard){
    ...
    })).filter(i=>/^[0-9a-f-]{36}$/i.test(String(i.product_id)));

    if(!canonical.length)throw new Error('CART_EMPTY_OR_NON_CANONICAL');
    ...
    await syncCheckoutCurrencyGate();
    ...
    const selectedLegal=await ensureCheckoutLegalAcceptance();
    void selectedLegal;

    const shippingQuote=await refreshVeloraShippingQuote();
    if(!shippingQuote?.ok)throw new Error('SHIPPING_CONFIGURATION_REQUIRED');

    const shippingAmount=Math.max(0,Number(shippingQuote.total_shipping||0));
    const couponCode=String(window.VELORA_ACTIVE_COUPON_CODE||'').trim().toUpperCase()||null;
    ...
    const {data,error}=await db.rpc('velora_create_order_with_commercials',{
      ...
      p_customer_notes:JSON.stringify({
        notes,
        country_code:country,
        payment_method:selected?.code||'gift_card'
      }),
      p_checkout_reference:(window.__VELORA_CHECKOUT_REFERENCE||(window.__VELORA_CHECKOUT_REFERENCE='VELORA-CO-'+(typeof crypto!=='undefined'&&typeof crypto.randomUUID==='function'?crypto.randomUUID():Date.now()+'-'+Math.random().toString(36).slice(2,10)))),
    ...
    if(
      ...
      const cleanup=await clearCanonicalCheckoutCart(canonical);
      STATE.cart=[];
      ...
      showToast(
        cleanup.failed
          ? `✅ Order #${data.order_number} created — paid by gift card. Cart sync needs retry.`
          : `✅ Order #${data.order_number} created — paid by gift card`,
        'success'
      );
    ...
    if(!selected?.id)throw new Error('PAYMENT_METHOD_REQUIRED');

    const methodResult=await db.rpc('velora_set_order_payment_method',{
      p_order_id:data.order_id,
      p_payment_method_id:selected.id
    });
    if(methodResult.error)throw methodResult.error;

    if(selected.code==='cash_on_delivery'){
      const cleanup=await clearCanonicalCheckoutCart(canonical);
      STATE.cart=[];
      saveToStorage(KEYS.CART,STATE.cart);
      updateCartBadge();
      showToast(
        cleanup.failed
          ? `✅ Order #${data.order_number} created — Cash on Delivery. Cart sync needs retry.`
          : `✅ Order #${data.order_number} created — Cash on Delivery.`,
        cleanup.failed?'warning':'success'
      );
      setTimeout(()=>navigateTo('orders'),900);
      return;
    }

    const attempt=await startPayment(data.order_id,selected.id,country);
    if(attempt?.checkout_url){
      window.location.assign(attempt.checkout_url);
      return;
    }
    ...
    showToast(
      cleanup.failed
        ? `✅ Order #${data.order_number} created — payment started. Cart sync needs retry.`
        : `✅ Order #${data.order_number} created — payment started.`,
      cleanup.failed?'warning':'success'
    );
    setTimeout(()=>navigateTo('orders'),900);
  }catch(e){
    console.error(e);
    showToast('❌ '+(e.message||'Payment could not be started'),'error');
  }finally{
    window.__VELORA_CHECKOUT_SUBMITTING=false;
    submitButtons.forEach(function(btn){btn.disabled=false;delete btn.dataset.veloraCheckoutSubmitting;});
  }
};
})();
