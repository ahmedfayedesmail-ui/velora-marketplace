...
function install46(){if(host46())return;const target=document.getElementById('sellerDashboardPage')||document.querySelector('[data-page="seller"].active')||document.querySelector('#page-orders.active')||document.querySelector('.customer-content.active');if(!target)return;const h=document.createElement('div');h.id='veloraTransactionLoop46';target.appendChild(h);setTimeout(()=>load46(),60)}
