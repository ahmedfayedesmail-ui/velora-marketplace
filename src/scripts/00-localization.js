...
/* Temporary on-page touch diagnostics.
   Enabled only with ?veloraDebug=cartTouch. Remove after root-cause capture. */
(function(){
    function startCartTouchDebug(){
        ...
        function write(label,e){
            const t=e && e.touches && e.touches[0] ? e.touches[0] : e;
            const x=typeof t.clientX==='number'?t.clientX:null;
            const y=typeof t.clientY==='number'?t.clientY:null;
            const point=x!==null&&y!==null?document.elementFromPoint(x,y):null;
            ...
            const row=[
                ...
                'visual='+(vv?vv.width:'na')+'x'+(vv?vv.height:'na')+
                  '@'+(vv?vv.offsetLeft:'na')+','+(vv?vv.offsetTop:'na')
            ].join(' | ');
            lines.push(row);
        ...
        ['pointerdown','touchstart','pointerup','touchend','click'].forEach(type=>{
            window.addEventListener(type,e=>write(type,e),{capture:true,passive:type!=='click'});
        });

        window.addEventListener('orientationchange',()=>write('orientationchange',new Event('orientationchange')),{capture:true});
        window.addEventListener('resize',()=>write('resize',new Event('resize')),{capture:true});

        write('READY',new Event('ready'));
    }

    const params=new URLSearchParams(location.search);
    if(params.get('veloraDebug')==='cartTouch'){
        if(document.readyState==='loading'){
            document.addEventListener('DOMContentLoaded',startCartTouchDebug,{once:true});
        }else{
            startCartTouchDebug();
        }
    }
...
(function(){
    let suppressNextCartClickUntil=0;
    ...
    function bindCartInteractions(){
        ...
        sidebar.addEventListener('click', function(event){
            ...
            if(Date.now() < suppressNextCartClickUntil){
        ...
        document.addEventListener('touchend', function(event){
            ...
            if(!touch || !sidebar.classList.contains('open')) return;
            ...
            for(const button of buttons){
                ...
                if(x >= rect.left && x <= rect.right && y >= rect.top && y <= rect.bottom){
                    suppressNextCartClickUntil=Date.now()+700;
                    event.preventDefault();
                    event.stopPropagation();
                    runCartAction(button);
                    return;
