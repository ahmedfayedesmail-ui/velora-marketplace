...
def main():
    p=argparse.ArgumentParser(description='Audit Velora static source for inline code and secret-like assignments. Exit 0=complete, 2=configuration/runtime error.')
    p.add_argument('--json',action='store_true',help='Emit JSON summary')
    args=p.parse_args()
    html=(ROOT/'src/index.html').read_text(errors='ignore')
    css=(ROOT/'src/styles/velora.css').read_text(errors='ignore') if (ROOT/'src/styles/velora.css').exists() else ''
    js_files=sorted((ROOT/'src/scripts').glob('*.js')) if (ROOT/'src/scripts').exists() else []
    ...
    checks={
      ...
      'inline_script_tags_remaining':len(re.findall(r'<script(?![^>]+src=)[^>]*>.*?</script\\s*>',html,re.I|re.S)),
